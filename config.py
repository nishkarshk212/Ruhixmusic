...
OWNER_USERNAME = getenv("OWNER_USERNAME", "˹🇯𝚊𝚢𝚍𝚎𝚗˼") #⚠️replace 
...
LOGGER_ID = int(getenv("LOGGER_ID", -1003713225825)) #⚠️REPLACE
DURATION_LIMIT_MIN = int(getenv("DURATION_LIMIT", 17000))
# External APIs - Get these from their respective providers
GPT_API = getenv("GPT_API")
DEEP_API = getenv("DEEP_API")
OWNER_ID = int(getenv("OWNER_ID", 8791884726)) #⚠️REPLACE
...
SUPPORT_CHANNEL = getenv("SUPPORT_CHANNEL", "@Tele_212_bots") #⚠️Replace
SUPPORT_CHAT = getenv("SUPPORT_CHAT", "@Titanic_world_chatting_zonee") #⚠️Replace
...
API_TIMEOUT = int(getenv("API_TIMEOUT", "15"))  # Reduced from 30 for faster response
REQUEST_TIMEOUT = int(getenv("REQUEST_TIMEOUT", "30"))  # Reduced from 60 for faster downloads
...
if SUPPORT_CHANNEL:
    if not re.match("(?:http|https)://", SUPPORT_CHANNEL) and not SUPPORT_CHANNEL.startswith("@"):
        raise SystemExit(
            "[ERROR] - Your SUPPORT_CHANNEL url is wrong. Please ensure that it starts with https:// or @"
        )

if SUPPORT_CHAT:
    if not re.match("(?:http|https)://", SUPPORT_CHAT) and not SUPPORT_CHAT.startswith("@"):
        raise SystemExit(
            "[ERROR] - Your SUPPORT_CHAT url is wrong. Please ensure that it starts with https:// or @"
        )
