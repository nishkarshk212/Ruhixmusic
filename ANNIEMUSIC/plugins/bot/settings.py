...
async def settings_back_markup(client, CallbackQuery: CallbackQuery, _):
    ...
    if CallbackQuery.message.chat.type == ChatType.PRIVATE:
        ...
        return await CallbackQuery.edit_message_media(
            InputMediaPhoto(
                media=random.choice(ANNIE_VID),
                caption=random.choice(AYUV).format(CallbackQuery.from_user.mention, app.mention, UP, DISK, CPU, RAM, served_users, served_chats),
            ),
...
# Skip Permission Handlers
@app.on_callback_query(filters.regex("SKIP_PERM") & ~BANNED_USERS)
@languageCB
async def skip_perm_menu(client, CallbackQuery: CallbackQuery, _):
    try:
        await CallbackQuery.answer()
    except:
        pass
    ...
    current_perm = await get_skip_perm(chat_id)
    buttons = skip_perm_markup(_, current_perm)
