...
skipperdb = mongodb.skipperm  # New database for skip permission
...
playtype = {}
skipmode = {}
skipperm = {}  # New memory cache for skip permission
mute = {}
...
async def get_skip_perm(chat_id: int) -> str:
    """Get skip permission setting: 'admin', 'member', or 'everyone'"""
    perm = skipperm.get(chat_id)
    if not perm:
        user = await skipperdb.find_one({"chat_id": chat_id})
        if not user:
            skipperm[chat_id] = "admin"  # Default to admin
            return "admin"
        skipperm[chat_id] = user.get("permission", "admin")
        return skipperm[chat_id]
    return perm


async def set_skip_perm(chat_id: int, permission: str):
    """Set skip permission: 'admin', 'member', or 'everyone'"""
    skipperm[chat_id] = permission
    await skipperdb.update_one(
        {"chat_id": chat_id}, 
...
async def is_skip_allowed(chat_id: int, user_id: int, is_admin: bool) -> bool:
    """Check if user can skip based on permission setting"""
    perm = await get_skip_perm(chat_id)
    
    if perm == "everyone":
        return True
    elif perm == "member":
        # Check if user is a member (not just admin)
