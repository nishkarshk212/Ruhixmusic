...
try:
    ...
    print(f"Failed to initialize Lexica API: {e}")
    Models = []
    api = None
...
async def selectModel(_:app,query:t.CallbackQuery):
    ...
    if promptData is None:
        return await query.edit_message_text("sᴏᴍᴇᴛʜɪɴɢ ᴡᴇɴᴛ ᴡʀᴏɴɢ @MRBROKN !!.")
    img_url = await ImageGeneration(modelId,promptData['prompt'])
    if img_url is None or img_url == 2 or img_url ==1:
        return await query.edit_message_text("sᴏᴍᴇᴛʜɪɴɢ ᴡᴇɴᴛ ᴡʀᴏɴɢ @MRBROKEN !!")
    elif img_url == 69:
